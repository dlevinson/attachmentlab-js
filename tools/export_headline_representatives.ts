...
async function main() {
  ...
  const outputRoot = path.join(repoRoot, 'results', 'research_program_20260407_medium', 'paper_replication');
  ...
  for (const scenario of source.config.scenarios) {
    ...
    const edges: RepresentativeEdge[] = state.edges
      .map((edge) => {
      ...
      .filter((edge): edge is RepresentativeEdge => edge !== null);
