...
export function sampleCapacity(
  ...
  switch (capacityMode as CapacityMode) {
    case 'homogeneous':
      ...
      break;
    case 'uniform':
      sampled = randomUniform(rng, capacityParams.low ?? minimum, capacityParams.high ?? minimum + 4);
      break;
    case 'lognormal':
      sampled = randomLogNormal(rng, capacityParams.mean ?? 1.5, capacityParams.sigma ?? 0.35);
      break;
    default:
      sampled = resolveCapacityValue(
        capacityValue ?? capacityParams.value,
  ...
  return Math.max(sampled, minimum);
}
