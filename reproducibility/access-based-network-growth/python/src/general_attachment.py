...
def sample_capacity(
    ...
    *,
    ...
    if mode == "constant":
        ...
        low = int(np.ceil(float(params["low"])))
        high = int(np.floor(float(params["high"])))
        if low > high:
            raise ValueError("Uniform capacity range contains no integer values.")
        value = float(rng.integers(low, high + 1))
    elif mode == "lognormal":
        mean = float(params["mean"])
        sigma = float(params["sigma"])
        value = float(np.floor(rng.lognormal(mean=mean, sigma=sigma) + 0.5))
    else:
        raise ValueError(f"Unsupported capacity mode: {mode}")
    return float(max(round(value), minimum))
