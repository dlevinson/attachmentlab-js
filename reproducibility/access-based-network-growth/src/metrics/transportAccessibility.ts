...
/**
 * Evaluate realized network accessibility on a common basis across growth
 * scenarios. Split-generated junctions remain in the shortest-path graph, but
 * only exogenously supplied seed and arrival nodes act as origins and unit-
 * weighted destinations.
 */
export function computeComparableNetworkAccessibility(
  nodes: NodeRecord[],
  edges: EdgeRecord[],
  {
    radius = 0.75,
  ...
  const origins = nodes.filter((node) => node.generatedBy !== 'split_crossing');
  if (origins.length === 0) {
    return {
      cumulativeById: {},
      gravityById: {},
  ...
  const adjacency = buildWeightedAdjacency(nodes, edges);
  const destinationIds = new Set(origins.map((node) => node.id));
  const cumulativeById: Record<string, number> = {};
  const gravityById: Record<string, number> = {};

  origins.forEach((origin) => {
    const distances = dijkstraDistances(origin.id, adjacency);
    let cumulative = 0;
    let gravity = 0;

    distances.forEach((distance, destinationId) => {
      if (destinationId === origin.id || !destinationIds.has(destinationId)) {
        return;
      }
      if (distance <= radius) {
        cumulative += 1;
      }
      gravity += Math.exp(-decay * distance);
    });

    cumulativeById[origin.id] = cumulative;
    gravityById[origin.id] = gravity;
  });
  ...
  return {
    cumulativeById,
    gravityById,
    meanCumulative: cumulativeEntries.reduce((sum, entry) => sum + entry[1], 0) / origins.length,
    meanGravity: gravityEntries.reduce((sum, entry) => sum + entry[1], 0) / origins.length,
    maxCumulative: bestCumulative?.[1] ?? 0,
    maxGravity: bestGravity?.[1] ?? 0,
    maxCumulativeNodeId: bestCumulative?.[0] ?? null,
    maxGravityNodeId: bestGravity?.[0] ?? null,
