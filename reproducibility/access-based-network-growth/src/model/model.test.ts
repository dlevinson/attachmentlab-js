...
describe('model core', () => {
  ...
  test.each([
    ['uniform', { low: 8, high: 24 }],
    ['lognormal', { mean: 2.647588722239781, sigma: 0.5 }],
  ] as const)('%s capacity mode assigns integer degree ceilings', (capacityMode, capacityParams) => {
    const state = runSimulation({
      ...createDefaultParams(),
      finalNodeCount: 100,
      capacityMode,
    ...
    expect(state.nodes.every((node) => Number.isInteger(node.capacity))).toBe(true);
  });
