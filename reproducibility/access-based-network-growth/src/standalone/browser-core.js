...
function sampleCapacity(rng, params, isSeed) {
  ...
  if (params.capacityMode === 'uniform') {
    const low = Math.ceil(params.capacityParams.low ?? minimum);
    const high = Math.floor(params.capacityParams.high ?? minimum + 4);
    if (low > high) {
      throw new Error('Uniform capacity range contains no integer values.');
    }
    return Math.max(Math.floor(randomUniform(rng, low, high + 1)), minimum);
  }
  if (params.capacityMode === 'lognormal') {
    return Math.max(Math.round(randomLogNormal(rng, params.capacityParams.mean ?? 1.5, params.capacityParams.sigma ?? 0.35)), minimum);
  }
  return Math.max(Math.round(resolveCapacityValue(params.capacityValue ?? params.capacityParams.value, params)), minimum);
}
...
function normalizedAccessSemantics(params) {
  return params.accessSemantics === 'seed'
    ? 'seed'
...
function accessDestinationWeight(node, semantics) {
  if (semantics === 'seed') {
...
function computeTransportAccessibility(nodes, edges, params) {
  ...
  const semantics = normalizedAccessSemantics(params);
  ...
  nodes.forEach((node) => {
    ...
    distances.forEach((distance, otherId) => {
      ...
      const opportunityWeight = destination ? accessDestinationWeight(destination, semantics) : 0;
...
function computeSiteAccessibilityRows(state, params, pool) {
  ...
  const rows = pool.candidates.map((candidate) => {
    ...
    state.nodes.forEach((node) => {
      ...
      const opportunityWeight = accessDestinationWeight(node, semantics);
      if (opportunityWeight <= 0) {
        return;
      }
      if (best <= radius) {
      ...
      gravity += opportunityWeight * Math.exp(-decay * best);
    });
    return {
      u: candidate.u,
      v: candidate.v,
...
function powerLawAlphaMle(sample, kMin) {
  ...
  const objective = (alpha) => alpha * logSum + sample.length * Math.log(hurwitzZeta(alpha, kMin));
  let left = 1.000001;
  let right = 100;
...
function exponentialFit(sample, kMin) {
  ...
  const meanExcess = shifted.reduce((sum, value) => sum + value, 0) / shifted.length;
  if (meanExcess < 0) {
...
function discreteLognormalProbability(value, mu, sigma, kMin) {
...
function lognormalFit(sample, kMin) {
  ...
  let mu = logs.reduce((sum, value) => sum + value, 0) / logs.length;
  const variance = logs.reduce((sum, value) => sum + (value - mu) ** 2, 0) / logs.length;
  let logSigma = Math.log(Math.max(Math.sqrt(variance), 0.1));
  let muStep = 1;
  let sigmaStep = 0.5;
  const negativeLogLikelihood = (candidateMu, candidateLogSigma) => {
