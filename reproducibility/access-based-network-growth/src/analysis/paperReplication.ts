...
export const paperTargets = {
  ...
  heterogeneous: {
    ...
    uniform_capacity: { maxDegree: 23.06, degreeGini: 0.355, shareAtCapacity: 0.0040, averageClustering: 0.0132 },
    lognormal_capacity: { maxDegree: 49.00, degreeGini: 0.381, shareAtCapacity: 0.0046, averageClustering: 0.0210 },
  },
} as const;
...
export async function runPaperReplicationSuite(options: PaperReplicationRunOptions = {}) {
  ...
  const sensitivityComparisonRows = sensitivityRows
    ...
    .map((row) => {
      const target = paperTargets.sensitivity[
        row.scenarioId as keyof typeof paperTargets.sensitivity
      ] as {
        meanEdgeLength?: number;
        averageClustering?: number;
        cyclomaticNumber?: number;
        averagePathLengthLargestComponent?: number;
      };
      return {
        ...row,
