...
import { computeComparableNetworkAccessibility } from '../metrics/transportAccessibility';
...
export function runSimpleBatch(config: SimpleBatchConfig): BatchResult {
  ...
  config.scenarios.forEach((scenario, scenarioIndex) => {
    ...
    for (let replication = 0; replication < config.replications; replication += 1) {
      ...
      const transportAccessibility = config.accessibilityEvaluation === 'common_exogenous_network'
        ...
        : engine.computeTransportAccessibility(state.nodes, state.edges, state.params);
      const metrics = {
  ...
  return {
    config: {
      ...
      accessibilityEvaluation: config.accessibilityEvaluation ?? 'configured',
    },
    runs,
