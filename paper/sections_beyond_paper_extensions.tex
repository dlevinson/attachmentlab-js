...
% immediately before \section{Discussion}. It is designed to add the new
% representative-graph/accessibility results and the focused planarity and
% accessibility-extension results without rewriting the main paper.
...
\subsubsection{Focused extension design}

The extension program was designed as a two-stage isolation exercise for
transport-oriented mechanism families beyond the baseline generalized model.
All results in this subsection come from the focused medium-scale suite, which
used $N=220$ final nodes and four replications per scenario. We report means
across replications.

In the first stage, we isolated \emph{planarity handling}: we held the arrival
process, capacity, and cost environment fixed, then varied only the crossing
rule across three matched free-geometry cases---\texttt{none},
...
cost pressure. Within that shared setting, we compared four cases: (i) a
reject-planarity control with no access weighting, (ii) split planarity with
target-only network access, (iii) split planarity with seed-based arrival and
target access, and (iv) split planarity with opportunity-based arrival and
target access. Across both stages we recorded admitted crossing candidates,
split events, generated intersection nodes, graph morphology, and realized
accessibility fields. Figure~\ref{fig:extension-design-schematic} summarizes
this logic: Panel A varies only the planarity rule, while Panel B varies only
the accessibility semantics within a split-capable planar regime.
...
reinforced. If space permits only one compact addition to the current paper,
this material is best inserted as a late Results subsection immediately before
the Discussion section, using Figures~\ref{fig:headline-access-gallery},
\ref{fig:extension-planarity}, and \ref{fig:extension-access} as the visual
sequence from validated baseline regimes to transport-growth extensions.
